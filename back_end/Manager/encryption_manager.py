...
def encrypt_file_content(data, filename):
    ...
    with open('D:/Private/Python/Sources/keys/encrypted_key.bin', 'wb') as file:
        file.write(encrypted_key)
    # save one time iv
    with open('D:/Private/Python/Sources/keys/iv.bin', 'wb') as file:
        file.write(iv)
...
def decryptFile(filePath, secretKey):
    ...
    with open('D:/Private/Python/Sources/keys/iv.bin', 'rb') as file:
        byte_dataiv = file.read()
    #print(byte_dataiv)

    # Open the file in write mode ('w') and write the new data
    with open('D:/Private/Python/Sources/keys/encrypted_key.bin', 'rb') as file:
        byte_dataencrypted_key = file.read()

    with open("D:/Private/Python/Sources/keys/recipient_private_key.pem", "rb") as key_file:
